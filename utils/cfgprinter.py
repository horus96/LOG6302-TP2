...
# test
outFile = open('textFiles/cfg.dot', 'wb')
...
tree = ET.parse('textFiles/doc.xml')
root = tree.getroot()
    ...
    for method in classe.findall(".function"):
        ...
        for bloc in method:
            ...
            if bloc.tag == "bloc":
                ...
                hasssa = False
                if namebis.__contains__(name_fun):
                    phis = []
                    pis = []
                    ...
                    for edge in bloc:
                        if edge.tag == "edge" and not(isNull):
                            ...
                            if nametargetbis.__contains__(name_fun) and (not nametargetbis.__contains__("exit") or bloc.findall(".ssa")[0].attrib.get("instruction").__contains__("return")):
                                #outFile.write(namebis)
                                #outFile.write("->")
